            ...
            if (!debug)
            {
                try
                {
                    ...
                    if (outResult == "OK")
                    {
                        ...
                        string res_ImportInvoices_Configuration = RSMNG.Plugins.Data.GetEnviromentVariable(crmServiceProvider.Service, "res_ImportInvoices_Configuration");
                        #endregion
                        ...
                        List<Shared.Document.ImportInvoiceDanea> invoicesDanea = new List<Shared.Document.ImportInvoiceDanea>();
                        ...
                        foreach (List<string> row in rows)
                        {
                            ...
                            string sPaymentTerm = configuration.fields.FirstOrDefault(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.Pagamento)) != null ? row[configuration.fields.First(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.Pagamento)).position] : null;
                            ...
                            string sBankDetails = configuration.fields.FirstOrDefault(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.CoordBancarie)) != null ? row[configuration.fields.First(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.CoordBancarie)).position] : null;
                            ...
                            string daSaldare = configuration.fields.FirstOrDefault(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.DaSaldare)) != null ? row[configuration.fields.First(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.DaSaldare)).position] : "0";
                            ...
                            string totNettoIva = configuration.fields.FirstOrDefault(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.TotNettoIva)) != null ? row[configuration.fields.First(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.TotNettoIva)).position] : "0";
                            decimal? netTotalExcludingVat = Helper.ValidateMoney(totNettoIva);

                            //Definisco l'iva
                            PluginRegion = "Definisco l'iva";
                            string iva = configuration.fields.FirstOrDefault(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.Iva)) != null ? row[configuration.fields.First(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.Iva)).position] : "0";
                            decimal? vat = Helper.ValidateMoney(iva);

                            //Definisco il totale documento
                            PluginRegion = "Definisco il totale documento";
                            string totaleDocumento = configuration.fields.FirstOrDefault(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.TotDoc)) != null ? row[configuration.fields.First(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.TotDoc)).position] : "0";
                            decimal? documentoTotal = Helper.ValidateMoney(totaleDocumento);

                            //Definisco la data
                            string data = configuration.fields.FirstOrDefault(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.Data)) != null ? row[configuration.fields.First(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.Data)).position] : "";
                            DateTime? date = Helper.ValidateDateTime(data);

                            //Definisco la data ultimo pag.
                            string dataUltimoPag = configuration.fields.FirstOrDefault(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.DataUltimoPag)) != null ? row[configuration.fields.First(f => f.name_invoice == nameof(Shared.Document.ImportInvoiceDanea.DataUltimoPag)).position] : "";
                            ...
                            crmServiceProvider.TracingService.Trace(invoicesDanea.ToString());
                        }
                        #endregion

                        #region Serializzo la struttura creata
                        PluginRegion = "Serializzo la struttura creata";
                        string invoicesDaneaOutput = RSMNG.Plugins.Controller.Serialize<List<Shared.Document.ImportInvoiceDanea>>(invoicesDanea, typeof(List<Shared.Document.ImportInvoiceDanea>));
                        byte[] jsonBytes = Encoding.UTF8.GetBytes(invoicesDaneaOutput);
                        ...
                        enDataIntegration.AddWithRemove(res_dataintegration.res_integrationaction, new OptionSetValue((int)GlobalOptionSetConstants.res_opt_integrationactionValues.DocumentiFatture));
                        enDataIntegration.AddWithRemove(res_dataintegration.res_name, $"{GlobalOptionSetConstants.res_opt_integrationtypeValues.Import.ToString()} - {GlobalOptionSetConstants.res_opt_integrationactionValues.DocumentiFatture.ToString()} - {localTime.ToString("dd/MM/yyyy HH:mm:ss")}");
                        ...
                        {
                            MimeType = "text/json",
                            FileName = $"{GlobalOptionSetConstants.res_opt_integrationtypeValues.Import.ToString()}_{GlobalOptionSetConstants.res_opt_integrationactionValues.DocumentiFatture.ToString()}_{localTime.ToString("dd_MM_yyyy_HH_mm_ss")}.json",
                            Id = enDataIntegrationId.ToString(),
